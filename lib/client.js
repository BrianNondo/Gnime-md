...
try {
    ...
    console.log("Connected to the Gnime_MD Whatsapp Bot By Astropeda")
} catch {
...
setTimeout(() => {
    ...
    async function syncdb() {
        let thumbbuffer;
        try {
        thumbbuffer = await getBuffer(THUMB_IMAGE);

        if (!Buffer.isBuffer(thumbbuffer)) {
            throw new Error("getBuffer did not return a valid Buffer");
        }
        } catch (err) {
        console.error("Error fetching thumb buffer:", err);
        thumbbuffer = Buffer.from(""); // fallback to empty buffer or handle as needed
        }

        const ChangePic = __dirname + "/assets/Gnime-logos.jpeg"
        await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/assets/Gnime-logos.jpeg");
	global.devp = fs.readFileSync(__dirname + "/assets/Gnime-logos.jpeg");
    ...
    global.srepo = 'https://github.com/Astropeda/Gnime-md'
    global.scap = 'ᴘᴏᴡᴇʀᴇᴅ ʙʏ ɢɴɪᴍᴇ ᴍᴅ '
    global.sgen = `╰┈➤ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ ɢɴɪᴍᴇ ${name.botname}`
    global.snam = 'ɢɴɪᴍᴇ ᴍᴅ'
    global.scate  = `
‗‗‗‗‗‗‗『 ${mztit}』━       
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                // if (botNumber!=='2348039607375@s.whatsapp.net') return
...
<html>
  ...
  <body>
    <section>
      Hello from "Astropeda"!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`Gnime-Md Server listening on port http://localhost:${port}!`));
